...
class ClarisseActions(HookBaseClass):
    ...
    def generate_actions(self, sg_publish_data, actions, ui_area):
        ...
        if "reference" in actions:
            action_instances.append( {"name": "reference", 
                                      ...
                                      "description": "This will add the item to the current context as a standard reference."} )

        if "import" in actions:
            action_instances.append( {"name": "import", 
                                      "params": None,
                                      "caption": "Import into Scene", 
                                      "description": "This will import the item into the current context."} )

        if "texture_node" in actions:
            action_instances.append( {"name": "texture_node",
                                      "params": None, 
                                      "caption": "Import Texture Map File", 
                                      "description": "Creates a file texture node for the selected item in the current context"} )
            
        if "texture_stream_node" in actions:
            action_instances.append( {"name": "texture_stream_node",
                                      "params": None, 
    ...
    def execute_action(self, name, params, sg_publish_data):
        ...
        # resolve path
        # toolkit uses utf-8 encoded strings internally and Clarisse API expects unicode
    ...
    def _do_import(self, path, sg_publish_data):
        ...
        image_extensions = ['.%s' % image_extension for image_extension in image_extensions] 
        ...
        import_project_extensions = [ext.lower() for ext in import_project_extensions]
    ...
    def _create_texture_node(self, path, sg_publish_data, stream=False):
        ...
        """
        image_extensions = ix.api.ImageIOFileFormat.get_supported_extensions()
        image_extensions = ['.%s'.lower() % image_extension for image_extension in image_extensions] 
